...
class UUID {
 public:
  ...
  constexpr explicit UUID(std::basic_string_view<T> uuid) {
    ...
    for (std::size_t i = 0; i < 8; i += 2) {
      ...
      data.data()[cnt] = static_cast<std::byte>(tmp + hexDigitToValue(uuid[i + 1]));
    }
  ...
  [[nodiscard]] friend std::basic_string<CharT, Traits, Allocator> to_string(const UUID &uuid) {
    ...
    for (std::size_t i = 0, index = 0; i < 32; i += 2) {
 ...
 private:
  ...
  constexpr static CharT NilUUIDString[36] = "00000000-0000-0000-0000-000000000000";
...
template<CharType T>
constexpr bool isValidUUID(std::basic_string_view<T> uuid) {
