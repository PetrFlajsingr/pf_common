...
template<unsigned int Dimensions = 3>
  requires(OneOfValues_v<Dimensions, 2, 3>)
struct BoundingBox {
  ...
  */
  ...
  bool operator==(const BoundingBox &rhs) const { return p1 == rhs.p1 && p2 == rhs.p2; }

  bool operator!=(const BoundingBox &rhs) const { return !(rhs == *this); }
  ...
  */
  ...
  inline friend BoundingBox operator*(const glm::mat3 &m, const BoundingBox &self)
    requires(Dimensions == 2)
  {
    return BoundingBox{(m * glm::vec3{self.p1, 1}).xy(), (m * glm::vec3{self.p2, 1}).xy()};
  }
  inline friend BoundingBox operator*(const glm::mat4 &m, const BoundingBox &self)
    requires(Dimensions == 3)
  {
    return BoundingBox{(m * glm::vec4{self.p1, 1}).xyz(), (m * glm::vec4{self.p2, 1}).xyz()};
  }

  inline friend BoundingBox &operator*=(const glm::mat3 &m, BoundingBox &self)
    requires(Dimensions == 2)
  {
    self = m * self;
    return self;
  }
  inline friend BoundingBox &operator*=(const glm::mat4 &m, BoundingBox &self)
    requires(Dimensions == 3)
  {
    self = m * self;
    return self;
...
inline float BoundingBox<2>::distance(const BoundingBox<2> &other) {
  const auto outterBB =
      BoundingBox<2>{{glm::min(p1.x, other.p1.x), glm::min(p1.y, other.p1.y)}, {glm::max(p2.x, other.p2.x), glm::max(p2.y, other.p2.y)}};
...
inline float BoundingBox<3>::distance(const BoundingBox<3> &other) {
  const auto outterBB = BoundingBox<3>{{glm::min(p1.x, other.p1.x), glm::min(p1.y, other.p1.y), glm::min(p1.z, other.p1.z)},
...
inline BoundingBox<2> BoundingBox<2>::combine(const BoundingBox<2> &other) const {
  return BoundingBox<2>{{glm::min(p1.x, other.p1.x), glm::min(p1.y, other.p1.y)}, {glm::max(p2.x, other.p2.x), glm::max(p2.y, other.p2.y)}};
}
