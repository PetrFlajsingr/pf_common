...
bool pf::math::BoundingBox<Dimensions>::operator==(
    const pf::math::BoundingBox<Dimensions> &rhs) const {
...
template<unsigned int Dimensions>
pf::math::RelativePosition
pf::math::BoundingBox<Dimensions>::contains(const pf::math::BoundingBox<Dimensions> &aabb) const {
...
bool pf::math::BoundingBox<Dimensions>::contains(Point point) const {
  if constexpr (Dimensions == 2) {
    ...
    return p1.x <= point.x && p1.y <= point.y && p1.z <= point.z && point.x <= p2.x
        && point.y <= p2.y && point.z <= p2.z;
  }
}
