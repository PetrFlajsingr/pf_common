...
class ViewFrustum {
 ...
 public:
  inline static ViewFrustum FromProjectionView(const glm::mat4 &viewMatrix, const glm::mat4 &projectionMatrix) {
    ViewFrustum result;
    ...
    for (int i = 0; i < 6; i++) { result.planes[i] = glm::normalize(result.planes[i]); }
    return result;
  }
  ...
  [[nodiscard]] RelativePosition contains(const BoundingBox<3> &aabb) const {
    static auto createPoint = [](auto &first, auto &second, auto &normal) {
      ...
      if (normal.x >= 0.0f) result.x = second.x;
      if (normal.y >= 0.0f) result.y = second.y;
      if (normal.z >= 0.0f) result.z = second.z;
    ...
    for (auto i = 0; i < 6; i++) {
      ...
      if (glm::dot(normal, createPoint(aabb.p1, aabb.p2, normal)) + pos < 0.0f) { return RelativePosition::Outside; }

      if (glm::dot(normal, createPoint(aabb.p2, aabb.p1, normal)) + pos < 0.0f) { return RelativePosition::Intersection; }
    }
